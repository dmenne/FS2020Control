...
public partial class MainWindow : Window
{
  ...
  MessageBoxRes ??= MessageBox.Show(
        "When you select <Yes>, the file will be opened." +
        "with the recommended program for extension xml or Notepad.\n" +
    ...
    switch (MessageBoxRes)
    {
      case MessageBoxResult.Yes:
        List<string> rec = RecommendedPrograms("xml");
        if (rec.Count > 0)
          Process.Start(rec[0], ct.FileName);
        else
  ...
  public static List<string> RecommendedPrograms(string ext)
  {
    List<string> names = [];
      ...
      if (rk != null)
      {
        ...
        if (mruList != null)
        {
          foreach (char c in mruList)
          {
            ...
            if (s != null && (s.Contains(".exe", StringComparison.CurrentCultureIgnoreCase)))
              names.Add(s);
          }
        }
      }
    ...
    {
      string? bk = String.Format(baseKey, name);
      if (bk != null)
      {
        RegistryKey? rk = Registry.CurrentUser.OpenSubKey(bk);
        if (rk != null)
          using (rk)
          {
            ...
            if (s != null && s != "")
            {
              s = s[1..s.IndexOf("\"", 2)]; //remove quotes
              paths.Add(s);
            }
          }
      }
